...
class IPut extends React.Component {
    ...
    onPropsChange() {
        ...
        if (mode==="segment"){
            this.props.onChange(this.state.value.map(val => isNaN(val) ? '' : val).join('.')+"/"+this.state.segment);
        }else {
    ...
    render() {
        ...
        const prefixInput = prefix ?
            <input className="prefix" style={{width: prefix.length * 40}} type="text" disabled
                   defaultValue={prefix}/> : "";

        const segment = mode === "segment" ?
            <div className="input-group" >
                <input className="prefix" disabled type="text" value={"/"} style={{width: 25}} />
                <input className="segment" type="number"
                   max={32} min={0} step={2}
                   onChange={(e) => this.setState({segment:e.target.value},() => this.onPropsChange())}
                   defaultValue={27}/>
            </div>
            :
            "";
